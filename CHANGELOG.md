...
- Added a bounded no-provider PTY self-test for exact nonce-bound main default, main bypass, and executor fallback denials; the acknowledged Bubblewrap-isolated provider run observed the first two denials but remained inconclusive for executor fallback.
