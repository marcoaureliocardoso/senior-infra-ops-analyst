...
- Added a bounded no-provider PTY self-test for exact nonce-bound main default, main bypass, and genuinely delegated executor fallback denials, with a content-free `SubagentStart` diagnostic; the corrected Bubblewrap-isolated provider run observed delegation and the first two denials but remained inconclusive because the delegated executor produced no `PreToolUse` audit.
