...
- Added a bounded no-provider PTY self-test for exact nonce-bound main default, main bypass, and genuinely delegated executor fallback denials, with a content-free `SubagentStart` diagnostic; the earlier Bubblewrap-isolated provider run observed the first two denials but remains inconclusive until the corrected fallback is run with separate authorization.
