...
| RV-96 | CodeQL declaration did not prove job wiring | Decoy jobs, exclusions, duplicate or non-canonical keys, scalars, or conditions preserved expected text without JavaScript analysis | Require the exact matrix, canonical keys, direct unconditional fail-closed `init`/`analyze`, and one direct `with.languages` binding in `jobs.codeql` | Remediated; final independent verification pending |
