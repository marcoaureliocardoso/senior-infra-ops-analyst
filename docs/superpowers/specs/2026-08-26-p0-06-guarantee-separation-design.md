...
| `DG-AUTHZ` | Deterministic | Untrusted content cannot modify package policy, native permissions, guard decisions, or credential and approval state through the package interface. | Installed artifacts, settings-preservation tests, guard tests, and authorization invariants. | The package does not control a compromised runtime or settings changed outside its boundary. |
| `DG-EFFECT` | Deterministic | Effects retain existing native tool, permission, command-guard, and operator-approval boundaries; the test deny hook blocks every proposed tool call. | Native-boundary, deny-hook, audit, and containment tests. | This does not prove the model will avoid tool proposals; deny-all is test-only. |
| `DG-EVIDENCE` | Deterministic | Validators and the active harness retain bounded structural evidence and delete disposable prompts, streams, model output, tool input, and canaries. | Parser, malformed-input, ownership, cleanup, and retention tests. | Provider and Claude Code transcripts may receive content outside package-controlled evidence. |
