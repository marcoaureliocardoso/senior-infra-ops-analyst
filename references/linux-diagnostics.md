...
| SAFE_READ_ONLY | `last -x | head -20` | Recent reboot/shutdown/login | Expected events | Unexpected reboot/shutdown | Correlate with incident start |
...
| SAFE_READ_ONLY | `top -b -n1 | head -40` | CPU/memory/process pressure | High `%wa` suggests I/O wait; high `%st` suggests hypervisor contention. |
...
| SAFE_READ_ONLY | `dmesg -T | tail -100` | Kernel/hardware errors | OOM, disk, NIC, filesystem, segfault clues. |
...
| SAFE_READ_ONLY | `find /var/log -xdev -type f -size +100M -printf '%s %p\n' 2>/dev/null | sort -n | tail -20` | Large logs | Large logs may be symptom; inspect before cleanup. |
| SAFE_READ_ONLY | `lsof +L1 2>/dev/null | head -50` | Deleted files held open | Space not freed until process closes file. Restart requires approval if production. |
...
| SAFE_READ_ONLY | `ss -tulpn` | Listening ports/processes | Service absent from expected port means app/bind issue. |
...
| SAFE_READ_ONLY | `journalctl -u <service> --since "1 hour ago" --no-pager` | Service logs | Exact errors guide fix; preserve messages. |
...
| Risk | Command | Verifies | Interpretation |
|---|---|---|---|
| SAFE_READ_ONLY | `grep -i "install\|upgrade\|remove" /var/log/dpkg.log* 2>/dev/null | tail -50` | Recent Debian package changes | Correlate package upgrades with incident. |
