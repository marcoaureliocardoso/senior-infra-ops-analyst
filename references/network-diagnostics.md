...
| SAFE_READ_ONLY + ACTIVE_PROBE | `nc -vz <target> <port>` | TCP port reachability | Open/refused/timeout separates service, host, and filtering clues. |
| SAFE_READ_ONLY + ACTIVE_PROBE | `traceroute <target>` | Path visibility | Incomplete hops may be policy; compare with port test. |
