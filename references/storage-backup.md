...
| SAFE_READ_ONLY | `du -xhd1 /var 2>/dev/null | sort -h` | Directory growth | Use same filesystem only with `-x`. |
| SAFE_READ_ONLY | `find / -xdev -type f -size +1G -printf '%s %p\n' 2>/dev/null | sort -n | tail -20` | Large files | Investigate before deletion. |
| SAFE_READ_ONLY | `lsof +L1 2>/dev/null | head -50` | Open deleted files | Space may not free until process restart. |
...
| SAFE_READ_ONLY | `Get-CimInstance Win32_LogicalDisk -Filter "DriveType=3" | Select DeviceID,Size,FreeSpace` | Drive capacity | Fast volume-level capacity view. |
| SAFE_READ_ONLY | `Get-WinEvent -FilterHashtable @{LogName='System'; StartTime=(Get-Date).AddHours(-24)} | Where-Object {$_.ProviderName -match 'disk|ntfs|stor'} | Select -First 100` | Disk/storage events | Hardware/FS clues with a bounded time window. |
...
| SAFE_READ_ONLY | `journalctl --since "24 hours ago" | grep -i backup | tail -100` | Backup-related logs | Generic clue when product unknown. |
| SAFE_READ_ONLY | `Get-Service | Where-Object {$_.Name -match 'backup|veeam|acronis|wbengine'}` | Windows backup services | Identify backup agent. |
| SAFE_READ_ONLY | `Get-WinEvent -LogName Application -MaxEvents 200 | Where-Object {$_.Message -match 'backup|VSS|shadow'}` | Backup/VSS events | Failed VSS writers, product errors. |
