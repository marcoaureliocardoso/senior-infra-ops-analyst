...
| SAFE_READ_ONLY | `Get-ComputerInfo | Select-Object WindowsProductName,WindowsVersion,OsHardwareAbstractionLayer,CsDomain` | OS/domain | Confirm role and domain. |
...
| Risk | Command | Verifies | Interpretation |
|---|---|---|---|
| SAFE_READ_ONLY | `ipconfig /all` | IP, DNS, DHCP, gateway | Wrong DNS/gateway commonly causes domain/service failures. |
| SAFE_READ_ONLY | `Get-NetIPConfiguration` | Interface/gateway/DNS | Cleaner structured network view. |
...
| SAFE_READ_ONLY | `Get-NetTCPConnection -State Listen` | Listening ports | Expected service not listening means service/app issue. |
...
| SAFE_READ_ONLY | `Get-Service | Where-Object {$_.Status -eq 'Stopped'} | Select -First 30` | Stopped services | Only useful with known baseline. |
| SAFE_READ_ONLY | `Get-WinEvent -LogName System -MaxEvents 100 | Select TimeCreated,ProviderName,Id,LevelDisplayName,Message` | System events | Correlate errors with incident window. |
| SAFE_READ_ONLY | `Get-WinEvent -LogName Application -MaxEvents 100 | Select TimeCreated,ProviderName,Id,LevelDisplayName,Message` | App events | App/service failures. |
...
| SAFE_READ_ONLY | `Get-Process | Sort-Object CPU -Descending | Select -First 10 Name,Id,CPU,WorkingSet` | Top processes | Identify heavy consumers without killing them. |

## 4. Windows update/change clues

| Risk | Command | Verifies | Interpretation |
|---|---|---|---|
| SAFE_READ_ONLY | `Get-HotFix | Sort-Object InstalledOn -Descending | Select -First 20` | Recent patches | Correlate with incident start. |
| SAFE_READ_ONLY | `Get-WinEvent -FilterHashtable @{LogName='System'; Id=1074,6005,6006,6008} -MaxEvents 20` | Shutdown/reboot events | Unexpected reboot or dirty shutdown. |
