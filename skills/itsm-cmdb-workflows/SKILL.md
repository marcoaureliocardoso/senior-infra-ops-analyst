...
8. Use shared risk vocabulary for ticket changes. A non-disruptive record
   update is `LOW_RISK_CHANGE` + `EXTERNAL_SIDE_EFFECT`; an approval action
   inherits the highest plausible base level of the execution it authorizes
   and adds `EXTERNAL_SIDE_EFFECT`.
